...
def regular_user(db, test_user, client: TestClient):
    user: User = test_user
    access_token: str = AuthService(db).create_access_token(
        user.username, user.user_key, user.is_admin, timedelta(30)
    )
...
def admin_user(db, test_user, client: TestClient):
    user: User = test_user
    access_token: str = AuthService(db).create_access_token(
        user.username, user.user_key, True, timedelta(30)
    )
    headers = {"Authorization": f"Bearer {access_token}"}
    client.headers.update(headers)
    yield user
