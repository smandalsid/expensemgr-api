...
def test_get_all_expenses(
    db, client, regular_user, currency: List[Currency], expense: List[Expense]
):
...
@pytest.mark.parametrize(
    "expense_id, currency_id, expected_response",
    [
        (
            [1],
            [1],
...
def test_get_expense(
    ...
    expense_ids = (
        "&expense_id=".join(map(str, expense_id)) if expense_id is not None else ""
    )
    currency_ids = (
        "&currency_id=".join(map(str, currency_id)) if currency_id is not None else ""
    )
